...
function consultarHistorico(sheet, dados) {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const sh = obterAbaHistorico(ss);
    const dh = sh.getDataRange().getValues();
    const r = [];
    for (let i = 1; i < dh.length; i++) {
      const l = dh[i];
      if (!l[0]) continue;
      const row = { dataHora: l[0], acao: l[1], frota: l[2], frenteAnterior: l[3], frenteNova: l[4], statusAnterior: l[5], statusNovo: l[6], dataRef: l[7], usuario: l[8] };
      r.push(row);
    }
    return { sucesso: true, dados: r, total: r.length };
  } catch (e) {
    return { sucesso: false, erro: e.toString(), dados: [] };
  }
}
...
function adicionarColhedora(sheet, dados) {
  try {
    const { frente, frota, data } = dados;
    const linha = encontrarLinhaFrota(sheet, frota);
    if (linha !== -1) return { sucesso: false, erro: `Frota ${frota} já existe` };
    const dataHoje = normalizarData_(data) || getDataAtual();
    const colData = garantirColunaData(sheet, dataHoje);
    const frenteVal = (frente && frente.trim()) ? frente : 'FRENTE - 08';
    sheet.appendRow([frenteVal, frota, '', '']);
