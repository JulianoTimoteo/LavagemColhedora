...
function lerGoogleSheets() {
    ...
    window[callbackName] = function (data) {
        ...
        if (data.dados && data.dados.length > 0) {
            ...
            registrosOriginais = dadosValidos.map(function (r, i) { return normalizarRegistro(r, i); });
            salvarLocal();
...
function carregarListaColhedoras() {
    ...
    const ordenados = registrosOriginais.slice().sort(function (a, b) { return a.frota.localeCompare(b.frota); });
    ...
    tbody.innerHTML = ordenados.map(function (r) {
        const frente = escapeHtml(r.frente || GRUPO_OFICINA);
        return '<tr>' +
...
function excluirColhedoraModal(id) {
    ...
    enviarAcao({ acao: 'excluir', frota: r.frota, data: r.data }, function () {
        ...
        registrosOriginais = registrosOriginais.filter(function (x) { return x.id !== id; });
        salvarLocal();
        salvarBancoLocal(montarBancoLocal());
        renderizarTudo();
        carregarListaColhedoras();
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Limpa cache local para garantir dados frescos da planilha
    try {
        localStorage.removeItem(CONFIG.chaveBanco);
        localStorage.removeItem(CONFIG.chaveOriginais);
        localStorage.removeItem(CONFIG.chaveUltimoSync);
    } catch (e) {}

    // Prioriza busca completa na planilha
